...
#include <fstream>
#include "Table.h"  // Предполагается, что класс Table вынесен в отдельный файл.
...
int main() {
    ...
    do {
        ...
        if (iResult > 0) {
            ...
            int var = std::stoi(command);  // Чтение команды в виде числа
            ...
            case 3:  // Отображение всех таблиц
                for (const auto& p : DB)
                    if (p.second.second)
                    {
            ...
            case 4:  // Добавление строки или обновление строки в таблице
                commonMessage = "Enter table name: ";
                send(ClientSocket, commonMessage.c_str(), commonMessage.length(), 0);
                recv(ClientSocket, recvbuf, recvbuflen, 0);
                recv(ClientSocket, recvbuf, recvbuflen, 0);
